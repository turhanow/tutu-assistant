...
> AI-native Telegram-ассистент от идеи «куда-нибудь на выходные» до проверенного плана поездки.

«Ту-да и обратно» сокращает путь от «хочу куда-нибудь на выходные» до конкретного решения: понимает запрос обычным языком, уточняет только действительно недостающие параметры, проверяет транспорт и проживание через Tutu и показывает до трёх совместимых вариантов с понятными компромиссами по цене, времени и удобству.
...
«Ту-да и обратно» объединяет этот процесс в один диалог:
...
- onboarding `/start → геопозиция или город → вдохновение или поиск`;
- отдельный AI-этап вдохновения с follow-up вопросами и переходом к проверенным карточкам;
- `/start`, `/reset`, `/help`, `/cancel` и подсказки команд Telegram;
...
### 4. Голос «Ту-да и обратно»

- единый tone of voice компетентного travel-приятеля без искусственного молодёжного сленга;
- одна управляемая лёгкая реплика максимум на диалог и только в безопасном контексте;
- нейтральные deterministic-тексты для цен, ошибок, privacy, feedback и оформления;
- `narration_v2` с проверкой grounded facts, стиля и запрещённых обещаний;
- feature flags для мгновенного возврата к нейтральному voice v1;
- аналитическая метка версии голоса для будущего A/B-теста без сохранения текста сообщений.
...
Имя, bio, описание и команды синхронизируются при запуске через Telegram Bot API. Фото
профиля API не поддерживает: установите [готовый аватар](assets/telegram-bot-avatar.png)
в BotFather командой `/setuserpic` для production-бота.
...
Опциональный smoke нового голоса на реально настроенной OpenAI-модели делает один платный
запрос и не выводит API key:

```bash
python scripts/run_discovery_evals.py --live-narration
```
...
Для платформенного мониторинга доступны `/health` (liveness) и `/readyz` (готовность каталога и провайдера). Маршрут `/healthz` сохранён как совместимый alias, но на стандартном домене Cloud Run может перехватываться Google Frontend.
...
«Ту-да и обратно» помогает подобрать и сравнить поездку. Актуальность цены и наличие подтверждаются провайдером в момент поиска и могут измениться до оформления. Бот не принимает оплату, не покупает билеты и не создаёт бронирование без явного действия пользователя на стороне Tutu.
