...
BOT_SHORT_DESCRIPTION = "Идеи и реальные варианты коротких поездок: билеты, отель, план и бюджет."
BOT_DESCRIPTION = (
    "«Ту-да и обратно» помогает вдохновиться поездкой или проверить готовый маршрут. "
    "Сопоставляет интересы с городами и активностями, проверяет билеты и отели через Tutu, "
    "сравнивает стоимость и собирает понятный план на два дня."
)

ROUTER_START = (
    "<b>Ту-да и обратно</b>\n"
    "Помогу придумать короткую поездку, проверить реальные билеты и отель, сравнить бюджет "
    "и собрать план без туристического квеста на выживание.\n\n"
    "Сначала укажите город отправления. Можно безопасно поделиться геопозицией через "
    "Telegram или написать город вручную. После отправки геопозиции бот отдельным "
    "сообщением подтвердит выбранный город. Если подтверждение не появилось, Telegram "
    "не передал геопозицию — выберите «Москва» или укажите город текстом. Точные "
    "координаты бот не сохраняет.\n\n"
    "Подбор новых направлений пока работает для выезда из Москвы; готовый маршрут можно "
    "проверить из другого города.\n\n"
...
DISCOVERY_START = (
    "<b>Давайте сначала поймём, какая поездка вам сейчас нужна.</b>\n"
    "Подбор новых направлений пока доступен только для выезда из Москвы. Готовый маршрут "
    "из другого города можно проверить через /newtrip.\n\n"
    "Нажмите на шаблон ниже, скопируйте его и замените подсказки в скобках:\n\n"
    "<code>Даты: [когда]. Длительность: [дни и ночи]. Хочу: [впечатления и активности]. "
    "Города: [если уже есть идеи]. Бюджет: [сумма]. Важно: [темп, дорога, отель].</code>\n\n"
    "Можно написать и свободно — например: «В эти выходные хочу старинный город с "
    "усадьбами, без суеты и с недорогим отелем». Сначала покажу идеи, а к билетам и "
    "сравнению перейдём, когда решитесь."
)
...
HELP = (
    ...
    "/reset — сбросить контекст и начать с выбора города\n"
